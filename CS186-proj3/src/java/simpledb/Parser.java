...
public class Parser {
	...
	public Query handleQueryStatement(ZQuery s, TransactionId tId) throws TransactionAbortedException, DbException,
		...
		LogicalPlan lp = parseQueryLogicalPlan(tId, s);// 逻辑层
		DbIterator physicalPlan = lp.physicalPlan(tId, TableStats.getStatsMap(), explain);// 非常关键的一步
		...
		if (physicalPlan != null) {
			...
			try {
				...
				m.invoke(null, (Operator) physicalPlan, lp.getTableAliasToIdMapping(), TableStats.getStatsMap());// updateOperatorCardinality
				c = Class.forName("simpledb.QueryPlanVisualizer");
				m = c.getMethod("printQueryPlanTree", DbIterator.class, System.out.getClass());
				m.invoke(c.newInstance(), physicalPlan, System.out);// printQueryPlanTree
			...
			if (s instanceof ZTransactStmt)
				...
				try {
					if (s instanceof ZInsert)
						...
						query = handleQueryStatement((ZQuery) s, curtrans.getId());// 注意，这里面要做查询优化
					else {
	...
	protected void start(String[] argv) throws IOException {
		...
		// 在这里SimpleDb会去加载数据
		// 这样使得SimpleDb的Catalog知道现在有哪些表，以及这些表的模式
		...
		if (!interactive) {
			...
			boolean quit = false;
			// if (true) {
			// line = "SELECT * from Actor a,Movie m,Casts c WHERE a.id = c.pid AND c.mid=
			// m.id;";
