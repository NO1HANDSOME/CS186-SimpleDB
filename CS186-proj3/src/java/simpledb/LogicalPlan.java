...
//    private Query owner;
	 ...
	 * to {@link Catalog#getDbFile(int)}). Aliases are added as base tables are
	 ...
	 * 
	 * @param field         The name of the over which the filter applies; this can
	 *                      be a fully qualified field (tablename.field or
	 ...
	 * @param constantValue the constant to compare the predicate against; if field
	 *                      is an integer field, this should be a String
	 *                      representing an integer
	 * @throws ParsingException if field is not in one of the tables added via
	 *                          {@link #addScan} or if field is ambiguous (e.g., two
	 ...
	 *                   qualified name (e.g., tableName.field or alias.field) or
	 *                   may be an unqualified unique field name. If the name is
	 *                   ambiguous or unknown, a ParsingException will be thrown.
	 ...
	 *                   may be an unqualified unique field name. If the name is
	 *                   ambiguous or unknown, a ParsingException will be thrown.
	 * @param joinField2 the subquery to join with -- the join field of the subquery
	 *                   is the first field in the result set of the query
	 * @param pred       The join predicate.
	 * @throws ParsingException if either of the fields is ambiguous, or is not in
	 *                          one of the tables added via {@link #addScan}
	 */
	 ...
	 *                          or if the field is ambiguous (appears in multiple
	 *                          tables)
	 */
...
//                int id = 
	 ...
	 * 
	 * @param t              The transaction that the returned DbIterator will run
	 *                       as a part of
	 * @param baseTableStats a HashMap providing a {@link TableStats} object for
	 *                       each table used in the LogicalPlan. This should have
	 *                       one entry for each table referenced by the plan, not
	 *                       one entry for each table alias (so a table t aliases as
	 *                       t1 and t2 would have just one entry with key 't' in
	 *                       this HashMap).
...
//                    id = 
	...
	public static void main(String argv[]) {
		 ...
		 * 
		 * JoinPredicate p = new JoinPredicate(1, Predicate.Op.EQUALS, 1); Join j = new
		 * Join(p, sf1, ss2);
		 */
		DbIterator j = null;
		try {
	...
	public TupleDesc getTupleDesc(String alia) {
		...
		if (tableMap.containsKey(alia)) {
			Integer tableid = tableMap.get(alia);
			td = Database.getCatalog().getDbFile(tableid).getTupleDesc();
		}
		return td;
	}
}