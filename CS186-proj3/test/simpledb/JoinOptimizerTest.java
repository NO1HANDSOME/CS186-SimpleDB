...
public class JoinOptimizerTest extends SimpleDbTestBase {
	...
	private double[] getRandomJoinCosts(JoinOptimizer jo, LogicalJoinNode js, int[] card1s, int[] card2s,
		...
		for (int i = 0; i < card1s.length; ++i) {
			...
			// assert that he join cost is no less than the total cost of scanning two
			// tables
	...
	public void estimateJoinCostTest() throws ParsingException {
		...
		jo = new JoinOptimizer(
				...
				new Vector<LogicalJoinNode>());
		// 1 join 2
		LogicalJoinNode equalsJoinNode = new LogicalJoinNode(t1Alia, t2Alia, Integer.toString(1), Integer.toString(2),
				Predicate.Op.EQUALS);
		...
		jo = new JoinOptimizer(
				p.generateLogicalPlan(tid,
						"SELECT * FROM " + tableName1 + " t1, " + tableName2 + " t2 WHERE t1.c1 = t2.c2;"),
				new Vector<LogicalJoinNode>());
		equalsJoinNode = new LogicalJoinNode(t2Alia, t1Alia, Integer.toString(2), Integer.toString(1),
				Predicate.Op.EQUALS);
		checkJoinEstimateCosts(jo, equalsJoinNode);
		// 1 join 1
		jo = new JoinOptimizer(
				...
				new Vector<LogicalJoinNode>());
		equalsJoinNode = new LogicalJoinNode(t1Alia, t1Alia, Integer.toString(3), Integer.toString(4),
				Predicate.Op.EQUALS);
		checkJoinEstimateCosts(jo, equalsJoinNode);
		// 2 join 2
		jo = new JoinOptimizer(
				...
				new Vector<LogicalJoinNode>());
		equalsJoinNode = new LogicalJoinNode(t2Alia, t2Alia, Integer.toString(8), Integer.toString(7),
				Predicate.Op.EQUALS);
		checkJoinEstimateCosts(jo, equalsJoinNode);
	}
	...
	/**
	 * Verify that the join cardinalities produced by estimateJoinCardinality() are
	 * reasonable
	 */
	@Test
	public void estimateJoinCardinality() throws ParsingException {
		 ...
		 * tableName2, Integer.toString(3), Integer.toString(4), Predicate.Op.EQUALS),
		 * stats1.estimateTableCardinality(0.8), stats2.estimateTableCardinality(0.2),
		 * false, false);
		 * 
		 * // We don't specify in what way statistics should be used to improve these
		 * estimates. // So, just require that they not be entirely unreasonable.
		 * Assert.assertTrue(cardinality > 800); Assert.assertTrue(cardinality <= 2000);
		 * 
		 * cardinality = j.estimateJoinCardinality(new LogicalJoinNode(tableName2,
		 * tableName1, Integer.toString(3), Integer.toString(4), Predicate.Op.EQUALS),
		 * stats2.estimateTableCardinality(0.2), stats1.estimateTableCardinality(0.8),
		 * false, false);
		 * 
		 * Assert.assertTrue(cardinality > 800); Assert.assertTrue(cardinality <= 2000);
		 */
		...
		// On a primary key, the cardinality is well-defined and exact (should be size
		// of fk table)
		// BUT we had a bug in lab 4 in 2009 that suggested should be size of pk table,
		// so accept either
	...
	/**
	 * Determine whether the orderJoins implementation is doing a reasonable job of
	 * ordering joins, and not taking an unreasonable amount of time to do so
	 */
	@Test
	public void orderJoinsTest() throws ParsingException, IOException, DbException, TransactionAbortedException {
		...
		// Note that there's no particular guarantee that the LogicalJoinNode's will be
		// in
		...
		// Set the last boolean here to 'true' in order to have orderJoins() print out
		// its logic
		...
		// "hobbies" only being at the two extremes, or "hobbies" being the outermost
		// table.
	...
	/**
	 * Test a much-larger join ordering, to confirm that it executes in a reasonable
	 * amount of time
	 */
	@Test(timeout = 60000)
